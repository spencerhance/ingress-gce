...
func TestToHttpHealthCheck(t *testing.T) {
	testCases := []struct {
		input    interface{}
	...
	for _, testCase := range testCases {
		result, _ := toHttpHealthCheck(testCase.input)
		if !reflect.DeepEqual(result, testCase.expected) {
			t.Fatalf("toHttpHealthCheck(input) = \ninput = %s\n%s\nwant = \n%s", pretty.Sprint(testCase.input), pretty.Sprint(result), pretty.Sprint(testCase.expected))
		}
	}
}
...
func TestToHttpsHealthCheck(t *testing.T) {
	testCases := []struct {
		input    interface{}
		expected *HttpsHealthCheck
	}{
		{
			computealpha.HttpsHealthCheck{},
			&HttpsHealthCheck{},
		},
		{
			computebeta.HttpsHealthCheck{},
			&HttpsHealthCheck{},
		},
		{
	...
	for _, testCase := range testCases {
		result, _ := toHttpsHealthCheck(testCase.input)
		if !reflect.DeepEqual(result, testCase.expected) {
			t.Fatalf("toHttpsHealthCheck(input) = \ninput = %s\n%s\nwant = \n%s", pretty.Sprint(testCase.input), pretty.Sprint(result), pretty.Sprint(testCase.expected))
		}
	}
}
